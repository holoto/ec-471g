...
createHtml=0   # This is now set by passing the 'html' argument when calling script. This automatically sets writeFiles to 1 also.
...
writeFiles=0   # This is now set by passing argument when calling script. This is automatically set when passing 'html' argument to script.
...
if [ -z "$gLogFile" ]; then
    gLogFile="/dev/null"
    gDumpFolderDiskPartitionInfo="${dumpFolderPath}"
fi
