...
Initialise()
{
    ...
    if [ -f "$gDDTmpFolder"/dd_csr ]; then
        local readCsrFromFile=$( cat "$gDDTmpFolder"/dd_csr )
        gCsrBits=$( echo "$readCsrFromFile" | tr -d ',' )
    else
        gCsrBits="111111"
    fi

    if [ -f "$gDDTmpFolder"/dd_loadedKexts ]; then
        local readLdDrvFromFile=$( cat "$gDDTmpFolder"/dd_loadedKexts )
        gLdDrvBits=$( echo "$readLdDrvFromFile" | tr -d ',' )
    else
        gLdDrvBits="1111"
    fi    
}
...
#overview_panel_two{ width: 340px; float: left; height: 74px; margin: 10px 10px 10px 30px; border-right-width: 1px; border-right-style: solid; border-right-color: #999; color: #DDD; overflow:auto; }
...
cat "$gDataDir/jquery-ui.structure.min.css" >> "$gCssDumpFile"
cat "$gDataDir/jquery-ui.theme.min.css" >> "$gCssDumpFile"
...
#fillUFS                               { background-color: #7515e1; border-top-color: #974ee8; border-bottom-color: #4e0e96; border-left-color: #333; border-right-color: #333; border-width: 2px; border-style: solid; padding: 2px 0 2px 0; }
#fillUFS:hover                         { background-color: #8a38e5; border-top-color: #a668eb; border-bottom-color: #6832a6; border-left-color: #333; border-right-color: #333; border-width: 2px; border-style: solid; padding: 2px 0 2px 0; }
#fillZFS                               { background-color: #e40bd7; border-top-color: #eb46e1; border-bottom-color: #98078f; border-left-color: #333; border-right-color: #333; border-width: 2px; border-style: solid; padding: 2px 0 2px 0; }
#fillZFS:hover                         { background-color: #e82fdc; border-top-color: #ee62e5; border-bottom-color: #a72c9f; border-left-color: #333; border-right-color: #333; border-width: 2px; border-style: solid; padding: 2px 0 2px 0; }
...
#fillNoHoverUFS                        { background-color: #7515e1; border-top-color: #974ee8; border-bottom-color: #4e0e96; border-left-color: #333; border-right-color: #333; border-width: 2px; border-style: solid; padding: 2px 0 2px 0; }
#fillNoHoverZFS                        { background-color: #e40bd7; border-top-color: #eb46e1; border-bottom-color: #98078f; border-left-color: #333; border-right-color: #333; border-width: 2px; border-style: solid; padding: 2px 0 2px 0; }
...
<script type=\"text/javascript\">" >> "$gJsDumpFile"
    cat "$gDataDir/jquery-2.1.4.min.js" >> "$gJsDumpFile"
    echo "
</script>" >> "$gJsDumpFile"
    ...
    echo "
<script>" >> "$gJsDumpFile"
    cat "$gDataDir/jquery-ui.min.js" >> "$gJsDumpFile"
    echo "
</script>" >> "$gJsDumpFile"
            ...
            do
                ...
                case "$tabname" in
                    ...
                    "audio")      if [ "${filesExtensionRemoved[$n]}" == "AudioCodecID" ]; then
                                      ...
                                      infoString="Dumped using <a href=\"http://www.projectosx.com/forum/index.php?showtopic=355\" target=\"_blank\">VoodooHDA's getdump tool</a>"
                                  fi ;;
...
WriteHtmlOverviewToFile()
{
    ...
    case "${passedSystemVersion}" in
        ...
        "Yos")  osVersion="Yosemite"
                osLogo="overview_logo_yos" ;;
        "Cap")  osVersion="El Capitan"
                osLogo="overview_logo_cap" ;; # To Do - Add new logo
  ...
  <div id=\"overview_topBand\">
    ...
    buildFile="$buildFile
      ...
      <div id=\"overview_panel_three\"><div id=\"${osLogo}\"></div><span class=\"text_overview_body\">Operating System<br /><strong>${osVersion}</strong><br />$passedOsVersion<br />${passedSic}</span></div>
...
DumpSystemOverview()
{
    ...
    if [ ${#tmpChipsetArr[@]} -gt 0 ]; then
        ...
        do
            ...
            [[ c -gt 0 ]] && gCodecID="${macGraphics}<br />"
            macGraphics="${macGraphics}"$( printf '%s\n\r' "Video: ${tmpChipset[$c]} <span class=\"text_overview_device\">[${tmpVendor}:${tmpDevice}] Rev.[${tmpRevision}]</span>" )
        done
    else
        macGraphics="${tmp[@]}"
    fi
    ...
    WriteHtmlOverviewToFile "$gtheprog" "$gVERS" "$timeStamp" "$systemVersion" "$macOsVer" "$macModel" "$macCpu" "$macGraphics" "${macMemory} ${memSpeed} ${memType}" "$gCodecID" "$gSic"
}
...
DumpHtmlAudio()
{
    ...
    if [ -f "$gDumpFolderAudio/VoodooHDAGetdump.txt" ] || [ -f "$gDumpFolderAudio/AudioCodecID.txt" ]; then
        ...
        ReadDirCreateTabsAddFileData "$gDumpFolderAudio" "audio"
        WriteEndingH3
...
DumpHtmlBootLog()
{
    ...
    if [ ! "$checkFileExistence" == "" ]; then
        echo "${gLogIndent}adding boot log" >> "${gLogFile}"
        InsertInternalSeparatorHTMLToFile "Boot Log"
        for file in "$gDumpFolderBootLog"/*_BootLog.txt
        do
            fileName+=("${file##*/}")
            if [[ "${fileName}" =~ "BootLog.txt" ]]; then
                ReadFileAndWriteCompleteSectionToHtml "$gDumpFolderBootLog/${fileName}" "Boot Log" "Dumped using the <a href=\"http://sourceforge.net/p/cloverefiboot/code/1905/tree/CloverPackage/utils/bdmesg/bdmesg.c\" target=\"_blank\">bdmesg tool</a>. Created by Kabyl, modified by JrCs from an idea by STLVNUB" "<a href=\"$finalDestination/${fileName}\" target=\"_blank\">View ${fileName} File</a>" "BootLog" "aBootLog" "" "collapseID_BootLog"
                break
            fi
        done
    fi
...
DumpHtmlSip()
{
    ...
    if [ -f "$gDumpFolderSip/SIP_status.txt" ]; then
        echo "${gLogIndent}adding SIP dump" >> "${gLogFile}"
